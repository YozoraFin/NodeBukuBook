...
export default function Livechat({ socket }) {
    ...
    const handleSubmitMessage = (e) => {
        ...
        chatData.pesan.push(document.getElementById('message').value)
        setMessage(messagedata)
        document.getElementById('message').value = ''
        addNewMessage(document.getElementById('message').value)
    }

    const addNewMessage = (msg) => {
        var chatdata = chat
        chatdata.pesan.push(msg)
        setChat(chatdata)
    }
    ...
    useEffect(() => {
        socket.on('sendNewMessage', (data) => {
            ...
            getNewMessage(data.data.id['_serialized'])  
        })
    }, [])
